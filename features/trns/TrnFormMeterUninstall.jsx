...
const TrnFormMeterUninstall = (props) => {
  console.log(`TrnFormMeterUninstall ----------------------`);
  const { meterData, validationSchema } = props;
  // console.log(`TrnFormMeterUninstall props`, props);
  // console.log(`TrnFormMeterUninstall meterData`, meterData);

  // console.log(
  //   `TrnFormMeterUninstall meterData`,
  //   JSON.stringify(meterData, null, 2)
  // );
  // console.log(`TrnFormMeterUninstall validationSchema`, validationSchema);

  const { sps } = useSps();
  // console.log(`sps`, sps);

  const [setSelected] = useState("Access");
  // console.log(`TrnFormMeterUninstall selected`, selected);
  ...
  // console.log(`TrnFormMeterUninstall trnState`, trnState);
  ...
  return (
    ...
    >
      {(formik) => {
        ...
        // console.log(
        //   `TrnFormMeterUninstall_ formik`,
        //   JSON.stringify(formik, null, 2)
        // );
        // console.log(
        //   `TrnFormMeterUninstall_ formik?.values?.ast?.astData?.astState`,
                ...
                >
                  <View style={{ gap: 15 }}>
                    ...
                    <FormikControl
                      ...
                      icon={<Octicons name="question" size={24} />}
                      options={formSelectOptions.uninstallCommentOptions}
                    />
                  </View>
                </PageSection>
              </>
...
export default TrnFormMeterUninstall;

const FormikValuesObserver = () => {
  useEffect(() => {
    // This effect will run whenever 'values' changes
    // console.log("Formik values changed:", values);
    console.log(`FormikValuesObserver formik`, JSON.stringify(formik, null, 2));
    // You can perform any action here, like:
    // - Sending data to an API
    // - Updating local state based on form values
    // - Triggering other side effects
  }, [formik]); // Dependency array ensures the effect runs only when 'values' changes
