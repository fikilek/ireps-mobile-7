...
const TrnFormMeterCheckIn = (props) => {
  // console.log(`TrnFormMeterCheckIn ----------------------`);
  const { meterData, validationSchema, spStores } = props;
  // console.log(`TrnFormMeterCheckIn spStores?.length`, spStores?.length);
  // console.log(`TrnFormMeterCheckIn meterData`, meterData);
  // const currentAstState = meterData?.ast?.astData?.astState?.state;
  // console.log(`currentAstState`, currentAstState);
  // console.log(
  //   `TrnFormMeterCheckIn spStores`,
  //   JSON.stringify(spStores, null, 2)
  // );
  // console.log(`TrnFormMeterCheckIn validationSchema`, validationSchema);
  ...
  // Get all stores owned by the user sp
  ...
  spStores?.forEach((store) => {
    if (store?.owner?.id === userSpId) {
      storeOptions?.push({
        key: store?.id,
        id: store?.id,
  ...
  // console.log(
  //   `TrnFormMeterCheckIn storeOptions`,
  //   JSON.stringify(storeOptions, null, 2)
  // );

  const [setSelected] = useState("Access");
  // console.log(`TrnFormMeterCheckIn selected`, selected);
  ...
  // console.log(`TrnFormMeterCheckIn trnState`, trnState);
  ...
  const onSubmit = useCallback(
    async (values, actions) => {
      ...
      //   }
      // } else {
      console.log(`Create a new meter CheckIn Trn`);
      // Create a new Trn
      ...
      console.log(`CheckIn createTrnResult : `, createTrnResult);

      if (createTrnResult?.data) {
        // show alert  and go back
        router.navigate("trns");
        // display success notice
        Alert.alert(`CheckIn Success`, `${createTrnResult?.data}`);
      }
      if (createTrnResult?.error) {
        // Show Alert only (Do not reroute - ussr must decide)
        Alert.alert(`CheckIn Failure `, createTrnResult.error);
        // }
      }
      // }
    },
                ...
                >
                  <View style={{ gap: 15 }}>
                    ...
                    <FormikControl
                      ...
                      name={`astData.media.astCheckIn`}
                      label="checkIn media"
                      placeHolder="CheckIn Media"
                      icon={<Octicons name="question" size={24} />}
                      media={media}
                      ml1="asts"
                    />
                ...
                >
                  <View style={{ gap: 15 }}>
                    ...
                    <FormikControl
                      ...
                      placeHolder="Checked In time"
                      icon={<Octicons name="question" size={24} />}
                      readOnly={true}
                    />

                    <FormikControl
                      control="formikSelectStore"
...
export default TrnFormMeterCheckIn;
