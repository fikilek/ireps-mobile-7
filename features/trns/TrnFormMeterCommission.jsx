...
const TrnFormMeterCommission = (props) => {
  // console.log(`TrnFormMeterCommission ----------------------`);
  const { meterData, validationSchema } = props;
  // console.log(`TrnFormMeterCommission spStores?.length`, spStores?.length);
  // console.log(`TrnFormMeterCommission meterData`, meterData);
  // const currentAstState = meterData?.ast?.astData?.astState?.state;
  // console.log(`currentAstState`, currentAstState);
  // console.log(
  //   `TrnFormMeterCommission meterData`,
  //   JSON.stringify(meterData, null, 2)
  // );
  // console.log(`TrnFormMeterCommission validationSchema`, validationSchema);
  ...
  // console.log(
  //   `TrnFormMeterCommission storeOptions`,
  //   JSON.stringify(storeOptions, null, 2)
  // );

  const [setSelected] = useState("Access");
  // console.log(`TrnFormMeterCommission selected`, selected);
  ...
  // console.log(`TrnFormMeterCommission trnState`, trnState);
  ...
  const onSubmit = useCallback(
    async (values, actions) => {
      ...
      // console.log(
      //   `TrnFormMeterCommission_ values *************** :`,
      ...
      console.log(`Commission createTrnResult : `, createTrnResult);

      if (createTrnResult?.data) {
        // show alert  and go back
        router.navigate("trns");
        // display success notice
        Alert.alert(`Commission Trn Success`, `${createTrnResult?.data}`);
      }
      if (createTrnResult?.error) {
        // Show Alert only (Do not reroute - ussr must decide)
        Alert.alert(`Commission Trn Failure `, createTrnResult.error);
        // }
      }
      // }
    },
                ...
                >
                  <View style={{ gap: 15 }}>
                    ...
                    <FormikControl
                      ...
                      name={`astData.media.commission`}
                      label="commission media"
                      placeHolder="Commission Media"
                      icon={<Octicons name="question" size={24} />}
                      media={media}
                      ml1="asts"
                    />
                ...
                >
                  <View style={{ gap: 15 }}>
                    ...
                    <FormikControl
                      ...
                      label="commissioned at time"
                      placeHolder="Commissioned at time"
                      icon={<Octicons name="question" size={24} />}
                      readOnly={true}
                    />
                    ...
                    <FormikControl
                      ...
                      placeHolder="Comment"
                      icon={<Octicons name="question" size={24} />}
                      options={formSelectOptions.commissionOptions}
                    />
                  </View>
                </PageSection>
              </>
...
export default TrnFormMeterCommission;
